...
```Clojure
(def comptador-funcs (let [comptador (atom 0)]
                       [#(identity @comptador)
                        #(reset! comptador 0)
                        #(swap! comptador + %) 
                        #(swap! comptador - %)]))

(let [[consulta, reset, incrementa, decrementa] comptador-funcs]
  (println (consulta))
  (println (incrementa 3))
  (println (decrementa 5))
  (println (reset))) 👉 nil
👁️ 0
👁️ 3
👁️ -2
...
el **mateix** context lèxic, en aquest cas l'`atom` anomenat `comptador`. Així, tenim un objecte 
_mutable_ forman part del context compartit per diverses funcions que, eventualment, poden consultar-lo
o modificar-lo.
