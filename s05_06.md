...
cal. Veiem un exemple senzill, on fem una funció `my-comp` per
...
(defn my-comp [& funcs]
  ...
  (let [ord-funcs (reverse funcs)]
    (letfn [(apply-comp [fns arg]
              (if (empty? fns) arg
                (recur (rest fns) ((first fns) arg))))] 
      (fn [arg]
        (apply-comp ord-funcs arg)))))
