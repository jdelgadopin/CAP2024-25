...
(`n` no és un paràmetre de la funció, tampoc és local), però aquesta funció <ins>_d'alguna manera sap_</ins>
...
(defn vèncer
  ...
  [vct _ parell]
  (concat (first parell) [(first vct)] (second parell)))  ;; combinem solucions
```
Aleshores...
```Clojure
...
Així es crea una **_cadena d'entorns_** que acaba en un entorn anomenat _global_. 
Aquesta cadena d'entorns que comença en l'entorn pare de la funció és el que considerem 
el seu **_context lèxic_**.
...
[11](https://jdelgadopin.github.io/CAP2024-25/s05_06.html#11)?
...
(let [x 3]
  (let [x  (g f x)] ;; aquí x diferents
    (let [f (g f 0)]
      f))) 👉 false
```
.tiny[**Exercici**: Per què a la versió Python `f` acaba valent 0 i la versió Clojure
acaba valent `false`?]
]
.col2[
![:scale 60%](figures/entorns_exemple_1.png)
]
]
...
Veiem un altre exemple en Python.red[*], fent servir 
el [Python Tutor](https://pythontutor.com):
...
.col2[
![:scale 90%](figures/entorns_exemple_2.png)
]]

.footnote[.red[*][Font](https://cs61a.org/exam/sp23/mt1/61a-sp23-mt1_sol.pdf)]
