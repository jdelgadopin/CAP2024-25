...
Fins ara hem vist _closures_ que capturaven un context lèxic format
...
a un valor, per tant el context lèxic estava format per
_lligams (entre noms i valors) immutables_. Això és una de les
propietats que fa de Clojure un llenguatge de programació adequat per
a la programació funcional.

Clojure, però, no és un llenguatge funcional _pur_, com ho pot ser
Haskell. Així, és possible que una _closure_ tingui i manipuli un
**_estat mutable_**. La diferència amb altres llenguatges és que a
Clojure cal fer-ho **_explícit_**. Clojure té varies maneres de
