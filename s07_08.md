...
servir `recur`, i tot i això no podrem aplicar TCO a la crida a la continuació.
En altre cas, però, no hi ha TCO. Ho hem vist amb l'exemple del
`fibonacci-cps`.
...
Encara que en Clojure el CPS no és especialment útil...

* Feu versions CPS de `my-map`, `my-filter`, i `my-reduce`, considerant que podeu partir de
les següents implementacions:
