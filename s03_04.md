...
`reduce` i `reductions` (anomenats `foldl` i `scanl` en altres llenguatges de programació)
són funcions extremadament útils. En veurem més exemples.
...
Discussió sobre `foldl`/`reduce`, `foldr` (sense equivalent a Clojure: `accumulate`?),
`scanl`/`reductions`, `scanr` (sense equivalent a Clojure: `accumulations`?)
