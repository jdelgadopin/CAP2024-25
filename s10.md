...
(defn arrel [x]
  (letfn [(arrels [x] (my-iterate #(float (/ (+ % (float (/ x %))) 2)) x))]
    (peek (vec (take 100 (arrels x)))))) ;; suposem que 100 termes són suficients
...
`mapv` i `filterv` són les versions no-_lazy_ de `map` i `filter`.
...
* Fes una funció per generar la seqüència _ordenada_ dels **nombres de Hamming**:
  ...
  ```
* Donat un nombre qualsevol seqüències, cadascuna ordenada de més
  petit a més gran, trobeu el nombre més petit que apareix a totes les
  seqüències. Les seqüències poden ser infinites, així que aneu amb
