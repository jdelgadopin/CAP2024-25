...
* Fes una macro `macro-fold` que, donada una funció de dos arguments
  `⊕`, faci un `fold` explícit, és a dir, la crida a `macro-fold`
  ...
  (⊕ x1 (⊕ x2 (⊕ x3 (⊕ x4 (⊕ x5 x0)))))
  ```
  Per simplificar, `macro-fold` només serà utilitzada amb seqüències _literals_: llistes
  -on no cal el `quote`- o vectors. No farem servir _símbols_ vinculats a llistes
  o vectors ni expressions que resultin en llistes o vectors
  (fer això seriosament és absurd, però és un bon exercici de macros).
