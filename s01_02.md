...
Els lligams locals només són visibles dins el cos del `let` i **_no es poden modificar_** dins el cos del `let`. 
...
## Lligams locals: La macro `letfn`

És similar al `let`, però permet definir **_funcions locals_**.
...
(defn arrel
    ...
    ([x] (arrel x 1.0))
    ([x aprox] 
       (letfn [(valor-absolut [x] ((if (> x 0) + -) x))
               (mitjana [x y] (/ (+ x y) 2))
               (prou-bo? [x aprox]
                    (let [diff (- (* aprox aprox) x)]
                        (< (valor-absolut diff) 0.001)))]
