...
## Exemple: Aproxima l'arrel quadrada
...
.tiny[.red[Source]: _The Joy of Clojure_, p. 93]
...
(keys my-hash-map)  👉 (:un :tres :dos)
(vals my-array-map) 👉 (2 3 1)
...
## Les Seqüències (V): Exemples (amb el que sabem fins ara.red[*]!)
...
Podem fer servir com a exemples alguns algorismes que haurien de ser vells coneguts... com la cerca en profunditat.
<br>
<br>
.tiny[[.red[Source]](https://dnaeon.github.io/graphs-and-clojure/)]
...
La cerca en profunditat (_Depth First Search_, DFS) des d'un node inicial. Retorna els nodes visitats, en
  ...
  (loop [pendents   [primer]  ;; vector amb conj/pop/peek = Pila
    ...
    (cond
      ...
      :else (let [actual    (peek pendents)
                  pendents  (into (pop pendents) (actual graf)) ;; Apila els veïns
                  visitats  (conj visitats actual)
                  recorregut (conj recorregut actual)]
              (recur pendents visitats recorregut)))))
              
(DFS G :A) 👉 [:A :C :D :F :G :E :B :X :Y]

```

.tiny[Estem acostumats a veure versions recursives de la cerca en profunditat. Aquesta versió, en ser iterativa,
