...
aquest fet: Quan troben una crida a funció en **_tail position_**, no cal crear cap estructura addicional 
(_stack frame_) per a aquella crida, i així no consumeixen espai. D'això se'n diu **_tail call optimization_** (TCO).

Clojure **no té TCO**, en general.
...
si totes les crides recursives estan en _tail position_). Per a això serveix `recur`:
...
Ara podem entendre l'ús de `recur` per definir les iteracions a Clojure. En realitat el que es fa és entendre un
bucle com una mena de funció recursiva final _implícita_.
