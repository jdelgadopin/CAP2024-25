...
Totes les col·leccions de Clojure que hem vist (llistes, vectors, diccionaris, conjunts) són
...
Les funcions principals per poder operar amb qualsevol objecte que es pugui considerar una
seqüència són:

`first`: Obtenir el primer element d'una seqüència

`rest`: Obtenir la seqüència original, menys el primer element. Retorna `()` (seqüència buida) si no hi ha més elements
